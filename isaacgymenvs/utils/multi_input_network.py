...
class Multi_Input_Replay_A2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            input_dict = kwargs.pop('input_shape')
            assert(type(input_dict) is dict)
            vec_input = input_dict["vec"]
            vec_num_inputs = vec_input.size()
            img_input = input_dict["img"]
            img_num_inputs = img_input.size()
    ...
    def build(self, name, **kwargs):
        net = Multi_Input_A2CBuilder.Network(self.params, **kwargs)
        return net
