...
class Bumpybot_passive_ds(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render=True):
        ...
        self.min_contact_thresh = self.cfg["env"]["minContactThreshold"]
        self,max_contact_speed = self.cfg["env"]["maxContactSpeed"]
        self.lambda_t = self.cfg["env"]["lambdaT"]
        self.lambda_N = self.cfg["env"]["lambdaN"]
        self.damping_lambda = torch.diag([self.lambda_t,self.lambda_n])
...
@torch.jit.script
def ds_control(last_action,action,net_cf,damping,max_contact_force,max_contact_speed,radius,mass,dt,root_states,num_assets):
    rot = root_states[:,3:7]
    _,_,yaw = get_euler_xyz(rot)
    heading = normalize_angle(yaw).unsqueeze(-1)

    theta = torch.arctan2(net_cf[:,1],net_cf[:,0]) - heading
    Fmag = torch.linalg.norm(force[:,:2])
    
    ctrl_pt_x = radius * torch.cos(theta)
    ctrl_pt_y = radius * torch.sin(theta)
    
    jacobian = torch.cat(
            (torch.cat((torch.ones_like(ctrl_pt_x),-ctrl_pt_y),dim=1),
            torch.cat((torch.zeros_like(ctrl_pt_x),ctrl_pt_x),dim-1)),dim=1)
    inv_jacobian = torch.cat(
            (torch.cat((torch.ones_like(ctrl_pt_x),ctrl_pt_y/ctrl_pt_x),dim=1),
            torch.cat((torch.zeros_like(ctrl_pt_x),1/ctrl_pt_x),dim=1)),dim=1)
    
    n_hat = torch.tensor([torch.cos(theta),torch.sin(theta)])
    t_hat = torch.tensor([-torch.sin(theta),torch.cos(theta)])

    Q = torch.tensor([n_hat,t_hat]).T
    D = Q @ damping @ Q.T #might need to use einsum

    V_prev = jacobian @ last_action #might need to use einsum
    V_cmd = jacobian @ action #might need to use einsum
    V_cmd_norm = torch.linalg.norm(V_cmd,dim=1)
    V_cmd = torch.where(V_cmd_norm>max_contact_speed,max_contact_speed*V_cmd/V_cmd_norm,V_cmd)

    release = n_hat.T @ V_cmd @ n_hat #might need to use einsum

    V = (dt / mass) * (-D @ V_prev + max_contact_force @ n_hat + Fmag @ n_hat) + t_hat.T @ V_cmd @ t_hat #might need to use einsum #negative Fmag term?
    V = torch.where(n_hat @ V_cmd < 0,V + release,V)
    #other differences to github implimentation

    return inv_jacobian @ V #might need to use einsum
