...
class Bumpybot_gazebo(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render=True):
        ...
        self.intersections = torch.zeros(self.num_envs,2).to(self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        steps = 24
        rad = 2
        self.path = rad*torch.tensor([[torch.cos(torch.tensor([2*np.pi*i/steps])),torch.sin(torch.tensor([2*np.pi*i/steps]))] for i in range(steps+1)])
        ...
        self.num_bodies = self.asset_bodies

        start_pose = gymapi.Transform()
        start_pose.p = gymapi.Vec3(*get_axis_params(0.6318758/2, self.up_axis_idx))+gymapi.Vec3(1,0,0)
    ...
    def _compute_observations(self):
        ...
        #print(self.intersections.size())
        #print(self.targets.size())
        self.obs_buf[:],self.intersections[:],self.targets[:] = compute_observations(
            self.root_tensor,
            self.path,
            self.targets,
            self.actions,
    ...
    def pre_physics_step(self, actions):
        ...
        self.torques = torch.zeros((self.num_envs*self.num_bodies, 3), device=self.device, dtype=torch.float)
...
@torch.jit.script
def get_dist_from_line(pt0,pt1,pt2):
    a = (pt1[:,1]-pt2[:,1]).view(1,-1)
    b = (pt2[:,0]-pt1[:,0]).view(1,-1)
    c = ((pt1[:,0]-pt2[:,0])*pt1[:,1]+(pt2[:,1]-pt1[:,1])*pt1[:,0]).view(1,-1)
    #print(a.size(),b.size(),c.size())
    num = a*pt0[:,0]+b*pt0[:,1]+c
    sq = a**2+b**2
    denom = sq**0.5
    x = (b*(b*pt0[:,0]-a*pt0[:,1])-a*c)/sq
    y = (a*(-b*pt0[:,0]+a*pt0[:,1])-b*c)/sq
    return (num/denom).view(-1,1),torch.cat([x.view(-1,1),y.view(-1,1)],dim=1)
#print(get_dist_from_line.code)
...
def compute_reward(
    ...
    # type: (Tensor, Tensor, Tensor, float, float, float, float, float, float, float, Tensor, Tensor, float, Tensor, float, float) -> Tuple[Tensor, Tensor, Tensor]

    rew_perp = 1 / (1 + torch.abs(obs_buf[:,0]))
    rew_par = obs_buf[:,1]/(max_vel*dt)
    rew_th_mot = 1 / (1 + torch.abs(obs_buf[:,4]))
    rew_th_dot = 1 / (1 + torch.abs(obs_buf[:,6]))
    rew_slew = 1 / (1 + torch.linalg.norm(torch.abs(obs_buf[:, 7:] - prev_action[:, :2]),dim=1))
    rew = rew_perp + rew_par + rew_th_mot + rew_th_dot + rew_slew
    total_reward = rew/5
    #print(total_reward)

    #delta_action = torch.sum(torch.abs(obs_buf[:, 7:] - prev_action[:, :2]),dim=-1) #weight first two vals by pi?
    ...
    #total_reward = actions_reward + pose_reward + heading_reward 
    ...
    # reset agents
    reset_conds = (progress_buf >= max_episode_length - 1) | (targets > path.size()[0]-2)
...
def compute_observations(
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, int, Tensor) -> Tuple[Tensor,Tensor,Tensor]
    ...
    ang_velocity = root_states[::num_assets, 12].view(-1,1)
    ...
    motion_heading_error = motion_heading - normalize_angle(torch.atan2(path[targets,0]-position[:,0],path[targets,1]-position[:,1])).unsqueeze(-1)

    speed = torch.linalg.norm(velocity[:,:2],dim=-1).view(-1,1)

    r_perp,intersection = get_dist_from_line(position[:,:2],path[targets-1],path[targets+1])
    r_par = torch.linalg.norm(intersection-last_intersection,dim=1)

    targets = torch.where(check_waypoints(position[:,:2]+path[targets],path,targets),targets+1,targets)
    r_par = torch.where(torch.linalg.norm(path[targets]-intersection,dim=1)<torch.linalg.norm(path[targets]-last_intersection,dim=1),r_par,-r_par).view(-1,1)

    obs = torch.cat((r_perp,r_par,heading,motion_heading,motion_heading_error,speed,ang_velocity,actions[:,:2]),dim=-1)
    #print(obs.size())
    #print(intersection.size())
    #print(targets.size())
    return obs, intersection, targets #[-1,9],[-1,2],[-1,1]